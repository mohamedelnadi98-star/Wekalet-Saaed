...
export async function createMultiplayer(directory){
 ...
 let data={};try{if(req.method==='POST'){let bytes=0,body='';for await(const chunk of req){bytes+=chunk.length;if(bytes>8192)throw Error('الطلب أكبر من المسموح');body+=chunk}data=JSON.parse(body||'{}')}}catch{send(400,{error:'طلب غير صالح'});return true}
 ...
 const token=String(req.headers.authorization||'').replace(/^Bearer /,'');const player=db.players.find(p=>p.token===hash(token));if(!player){send(401,{error:'سجل وكالة على هذا الخادم أولًا'});return}
 ...
 const shipment={id:randomUUID(),nonce:data.nonce,from:player.id,to:data.to,pid:data.pid,qty:data.qty,life:data.life,claimed:false};db.shipments.push(shipment);await persist();send(201,{shipment});return}
 ...
 }catch(e){db=before;send(400,{error:e.message})}};
