...
const analytics=await createAnalytics(path.join(process.env.DATA_DIR||path.join(root,'server-data'),'metrics'));
http.createServer(async(req,res)=>{try{const url=new URL(req.url,'http://localhost');if(await analytics(req,res,url))return;if(await multiplayer(req,res,url))return;let relative=decodeURIComponent(url.pathname);if(relative==='/')relative='/index.html';const file=path.resolve(root,'.'+relative);if(!file.startsWith(root+path.sep)||relative.includes('..')||relative.startsWith('/node_modules/')||relative.startsWith('/tests/')||relative.startsWith('/private-tools/')||relative.startsWith('/server-data/')||!mime[path.extname(file)])throw Error();if(!(await stat(file)).isFile())throw Error();const data=await readFile(file);res.writeHead(200,{'Content-Type':mime[path.extname(file)]||'application/octet-stream','Cache-Control':'no-cache','X-Content-Type-Options':'nosniff'});res.end(data)}catch{res.writeHead(404);res.end('Not found')}}).listen(port,process.env.HOST||'127.0.0.1',()=>{const url='http://localhost:'+port;console.log('وكالة سعيد: '+url);if(process.argv.includes('--open')&&process.platform==='win32')spawn('cmd',['/c','start','',url],{stdio:'ignore'})});
