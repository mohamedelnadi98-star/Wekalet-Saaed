...
buildWorld(){const hemi=new T.HemisphereLight(0xdaece8,0x68543d,2);this.hemi=hemi;this.scene.add(hemi);this.sun=new T.DirectionalLight(0xffd299,3.6);this.sun.position.set(-18,30,12);this.sun.castShadow=true;this.sun.shadow.mapSize.set(2048,2048);this.sun.shadow.camera.left=-32;this.sun.shadow.camera.right=32;this.sun.shadow.camera.top=28;this.sun.shadow.camera.bottom=-28;this.sun.shadow.normalBias=.035;this.sun.shadow.bias=-.00015;this.scene.add(this.sun);const fill=new T.DirectionalLight(0xc0e6ee,1.15);fill.position.set(15,10,-20);this.fill=fill;this.scene.add(fill);
 ...
 this.interact('dialogue','حسن • اتكلم',4.8,2.5,5.6,5,5);this.racks=[];for(const x of [1,5,9]){this.rack(x,-6.6);this.obstacle(x,-6.6,3,1.7);this.racks.push({x,z:-6.6});}
 this.coolGroup=new T.Group();this.scene.add(this.coolGroup);this.box(2.8,3.4,2.5,12.15,1.7,-6.45,'dark',this.coolGroup);this.box(2.55,3.1,.1,12.15,1.6,-5.14,'metal',this.coolGroup);for(const x of [11.15,13.1])this.box(.08,.7,.15,x,1.6,-5.02,'brass',this.coolGroup);this.label('غرف الحرارة',1.5,.48,12.15,2.8,-5.04,'#bbe4da','#275655',this.coolGroup);this.obstacle(12.15,-6.45,2.8,2.5);
 ...
 this.truck=this.makeTruck();this.truck.position.set(1.3,0,7.3);this.scene.add(this.truck);this.obstacle(1.3,7.3,5.2,2.5);
 ...
 this.gateBar=this.box(.14,.18,5.5,0,1.6,0,'teal',this.gate);
 ...
 this.box(220,.3,220,0,-1.55,0,'water');this.box(100,.25,15,2,-1.05,29,'concrete');for(let x=-48;x<60;x+=9){this.box(7,3+Math.abs(x%5),4,x,.8,-28,'wall');this.box(7.2,.22,4.2,x,2.5+Math.abs(x%5)/2,-28,'teal');}
