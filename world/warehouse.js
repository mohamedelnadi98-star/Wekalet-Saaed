...
carton(x,y,z,scale=1,parent=this.scene,color){let g=new T.Group();g.position.set(x,y,z);g.scale.setScalar(scale);parent.add(g);this.box(.75,.58,.58,0,.29,0,color||'box',g);this.box(.13,.012,.59,0,.586,0,'edge',g);this.box(.13,.58,.012,0,.29,.297,'edge',g);this.box(.22,.15,.015,-.17,.33,.306,'paper',g);for(let i=0;i<4;i++)this.box(.015,.1,.018,-.24+i*.035,.33,.319,'dark',g);return g},
...
updateWarehouseWork(dt,s){
 ...
 if(this.truck.userData.door){const target=s.carry||s.pickingJob||s.cargo.length?-1.1:0;this.truck.userData.door.rotation.z+=(target-this.truck.userData.door.rotation.z)*Math.min(1,dt*4)}
},
