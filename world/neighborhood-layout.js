...
export function neighborhoodLayout(){
 const streets=[{x:80,z:7.3,w:135,d:5},{x:22,z:8,w:5,d:59},{x:59,z:35,w:79,d:5},{x:96,z:8,w:5,d:59},{x:59,z:-19,w:79,d:5}];
 streets.push({x:118,z:49,w:68,d:5},{x:148,z:34,w:5,d:104},{x:96,z:34,w:5,d:104},{x:118,z:76,w:68,d:5});
 ...
 for(let i=0;i<clients.length;i++){const p=clients[i],candidates=[];for(let dz=-22;dz<=22;dz+=2)for(let dx=-22;dx<=22;dx+=2){const b={x:p.x+dx,z:p.z+dz,w:6.2,d:4.8,client:i};if(b.x<26||b.z< -10||b.z>80)continue;candidates.push(b)}candidates.sort((a,b)=>Math.hypot(a.x-p.x,a.z-p.z)-Math.hypot(b.x-p.x,b.z-p.z));const b=candidates.find(clear);if(!b)throw Error('No connected customer plot '+i);b.rotation=p.z>=b.z?0:Math.PI;b.front={x:b.x,z:b.z+(b.rotation? -2.5:2.5)};plots.push(b);shops.push(b)}
 plots.push({x:70,z:43,w:8.4,d:7.5,facility:'branch'});
 for(const z of [-10,-2,17,26,43,59,68])for(let x=30;x<=140;x+=10){const b={x,z,w:8.4,d:7.5,floors:2+(plots.length%3)};if(clear(b))plots.push(b)}
