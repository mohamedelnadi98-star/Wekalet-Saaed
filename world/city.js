...
export const city={
 ...
 for(const [x,z,name]of [[27,-14,'حي السوق'],[51,26,'حي النخيل'],[77,43,'حي التوسع']]){this.cyl(.045,.065,2.7,x,1.2,z,'metal',this.cityGroup);this.label(name,3.1,.55,x,2.5,z,'#f0e5ce','#2c5c55',this.cityGroup)}
 ...
 this.box(6.2,.10,4.8,0,-.02,0,'sidewalk',g);const customer=this.human('shirt',(i+2)%9);customer.position.set(-1.3,0,-2.8);customer.rotation.y=Math.PI;customer.userData.action='customer:'+i;g.add(customer);g.userData.customer=customer;g.userData.renewal=new T.Group();g.add(g.userData.renewal);return g;
 },
 updateCity(dt,s){
