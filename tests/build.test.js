...
test('portable HTML rewrites all modules to embedded URLs, and PWA caches every module/texture',()=>{
 ...
 const sw=fs.readFileSync(path.join(root,'service-worker.js'),'utf8'),assets=JSON.parse(sw.match(/ASSETS=(\[[^;]+\])/)[1]);for(const asset of assets)assert(fs.existsSync(path.join(root,asset)),'Missing precache asset: '+asset);assert(assets.includes('./network.js'));assert(assets.includes('./guide.js'));assert(assets.includes('./Guide.html'));assert(assets.includes('./world/city.js'));assert(assets.includes('./world/effects.js'));assert(assets.includes('./assets/icon-512.png'));assert(sw.includes("url.pathname.includes('/api/')"));assert(!assets.some(a=>a.includes('server-data')));
});
